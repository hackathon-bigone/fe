...
const R_Detail = () => {
  ...
  const handleHeart = async () => {
    try {
      const response = await axios.post(
        `https://43-203-179-188.sslip.io/recipe/${id}/like`,
        {},
        {
          headers: {
  ...
  const handleScrapClick = async () => {
    try {
      const response = await axios.post(
        `https://43-203-179-188.sslip.io/recipe/${id}/scrap`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `https://43-203-179-188.sslip.io/recipe/${id}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
    ...
    const fetchScrap = async () => {
      try {
        const res = await axios.get(
          "https://43-203-179-188.sslip.io/mypage/recipe-scrap",
          {
  ...
  const handleDeleteClick = async () => {
    try {
      await axios.delete(`https://43-203-179-188.sslip.io/recipe/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  return (
    <R.Container>
      ...
      <R.Content>
          ...
          {component.categories &&
            component.categories.map((category) =>
              categoryLabels[category] ? (
                <R.D_State key={category}>{categoryLabels[category]}</R.D_State>
              ) : null
            )}
        </R.Wrapper>
        ...
        <R.PostURL>
          <p>레시피 링크</p>
          {Array.isArray(component.recipeLinks) &&
          component.recipeLinks.length > 0 ? (
            component.recipeLinks.map((link) => (
              <p key={link.recipelinkId}>
                <a
                  href={link.recipelinkUrl}
                  target="_blank"
      ...
      <R.ContentBar>
        <R.Tap
          id="ingredients"
          $selected={isSelected === "ingredients"}
          onClick={() => handleSelectClick("ingredients")}
        >
          재료
        </R.Tap>
        <R.Tap
        ...
        <R.Ingredients>
          {ingredients.map((ingredient) => (
            <div id="wrapper" key={ingredient.ingredientId}>
              ...
              <img
                id="line"
                src={`${process.env.PUBLIC_URL}/images/Line_in.png`}
                alt="line"
              />
            ...
            <R.RecipeStep key={step.stepId}>
              <R.PicStep>
                <img
                  src={
                    step.stepImageUrl
                      ? `https://43-203-179-188.sslip.io/uploads/r?key=${step.stepImageUrl}`
                      : "/images/scrap.svg"
                  }
                  alt="임시"
                />
