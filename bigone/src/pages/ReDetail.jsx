...
const R_Detail = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`https://43-203-179-188.sslip.io/recipe/${id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
    ...
    const fetchScrap = async () => {
      try {
        const res = await axios.get("https://43-203-179-188.sslip.io/mypage/recipe-scrap", {
        ...
        <R.Recipe>
          ...
          {step?.map((step) => (
            <R.RecipeStep key={step.stepId}>
              <R.PicStep>
                <img src={step.stepImageUrl ? `https://43-203-179-188.sslip.io/uploads/r?key=${step.stepImageUrl}` : "/images/scrap.svg"} alt="임시" />
              </R.PicStep>
